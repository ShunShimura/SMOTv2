...
from sliced_multi_object_tracker.detector.ultralytics.data.augment import LetterBox
from sliced_multi_object_tracker.detector.ultralytics.utils import LOGGER, SimpleClass, ops
from sliced_multi_object_tracker.detector.ultralytics.utils.plotting import Annotator, colors, save_one_box
from sliced_multi_object_tracker.detector.ultralytics.utils.torch_utils import smart_inference_mode
...
class Results(SimpleClass):
    ...
    def save(self, dir:Path, *args, **kwargs):
        """Save annotated results image."""
        if not dir.exists():
            dir.mkdir()
        if self.boxes.data.shape[0] != 0:
            self.plot(save=True, filename=str(dir/Path(self.path).name) ,*args, **kwargs)
    ...
    def save_txt(self, dir:Path, basename, save_conf=True):
        """
        ...
        if texts:
            with open((Path(dir)/basename).with_suffix('.txt'), "a") as f:
                f.writelines(text + "\n" for text in texts)
